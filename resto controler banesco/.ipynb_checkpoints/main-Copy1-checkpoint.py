...
class PaymentBanescoController(http.Controller):
    ...
    def index(self, **kw):
      response_content = self.setup_completion_with_card_number()
      ...
      return http.request.render('payment_banesco.authenticate', {
            ...
            'device_data_collection_url': response_content.device_data_collection_url,
    ...
    def del_none(self,d):
        for key, value in list(d.items()):
            if value is None:
                ...
                self.del_none(value)
        return d
    def setup_completion_with_card_number(self):
        ...
        paymentInformationCardNumber = "4456530000001005"
    ...
    def banesco_payment(self, **kwargs):
        ...
        '''
        #para autenticacion 3ds
        auth_content = self.authenticate_card(**post)
        with open('/home/odoo/src/user/respuesta_auth.txt', 'w') as temp_file:
            temp_file.write("%s\n" %  auth_content)
            
            
        if auth_content.status=='AUTHENTICATION_SUCCESSFUL':
            response_content = self.simple_authorizationinternet(False, **post)#solo autorizacion
        else:
            response_content = auth_content
        with open('/home/odoo/src/user/respuesta_tarjeta.txt', 'w') as temp_file:
            temp_file.write("%s\n" %  response_content)
        '''
        response_content = self.simple_authorizationinternet(False, **post)
